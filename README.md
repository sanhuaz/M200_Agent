...
- Git 仅跟踪 `persona/.gitkeep`，本机 `persona/*.json`、私有文档、数据库、日志和真实评测结果均不进入公开仓库。
- FastAPI 与前端包版本同步更新为 `0.4.3`；完整检查通过 145 项测试，另有 1 项 Windows 软链接权限测试跳过。
...
- 新增 150 题题集生成器、Schema 和评测器；情绪提示词升级为共享判定指南，评测器增加严格 Top-1、Top-3
  命中率、排名分布和安全样本剔除口径；已完成全量 150 题 DSvision 复测，结果保存在忽略目录。
- 新增未覆盖原题集的 `companion-p0-gold-v2` 版本化金标审计、60 条固定未见日常留出集，以及带模型/提示/分类器/金标版本
  隔离的评测缓存；增加逐类指标、每标签 P/R/F1、混淆矩阵、候选频次和 Top-1 错误拆分。
- 增加可选 `companion-analysis-v4` / `companion-emotion-only-v3` 实验提示、`emotion-classifier-v3` 和高精度显式情绪锚点，
  支持否定、引用、假设及第三方情绪过滤；当前生产默认仍保持 `companion-analysis-v3` / `emotion-classifier-v2`。
- 30 条 DSvision 筛选复测中，v2 gold-v2 基线 Top-1 为 60.00%、Top-3 为 96.67%，v3 为 63.33% / 86.67%，未达到预设切换门槛，
  因此未继续执行后续付费复测，也未切换生产默认。完整项目检查通过。
- 后续 v5/v4 候选证据驱动实验未通过首轮门槛，已清理对应实验实现、测试引用及两份 v5 评测运行目录；
  当前生产仍固定为 `companion-analysis-v3` / `emotion-classifier-v2`，历史 v2 报告未覆盖、未回填。
- 新增 `scripts/generate_companion_daily50.py`，生成独立的 50 条低风险日常题（学业 8、求职 8、职场 8、
  人际 8、孤独 7、积极 4、模糊/混合 7），并与 `gold-v2` 的原 120 条日常题合并为 170 条；原题集和历史结果保持只读。
- 使用当前生产结构化链路和 DSvision 对 170 条日常题完成 170 次全新真实调用：170/170 成功、Schema 100%、
  Top-1 65.88%、Top-3 87.65%、Micro-F1 0.7024、Macro-F1 0.6628、Exact Match 29.41%；支持需求准确率 61.18%、
  策略准确率 56.47%（96/170）。策略按场景为：积极事件 85.71%、孤独 68.18%、人际 60.71%、学业 53.57%、
  职场 53.57%、求职 46.43%、模糊/混合 40.91%。
- 同时生成与此前情绪-only 报告同口径的 170 条复测：复用旧 120 条真实结果、对新增 50 条实际调用模型，
  `companion-emotion-only-v2` / `emotion-classifier-v2` 的 Top-1 70.59%、Top-3 88.82%、Micro-F1 0.6890；
  该口径只评测情绪，不评测支持需求和策略，不与结构化结果直接横向比较。
- 本轮结果表明候选覆盖明显高于主情绪排序：结构化 Top-1 与 Top-3 相差 21.77 个百分点，58 个 Top-1 错误中 37 个为候选排序偏差、
  21 个为完全漏标；积极事件最稳定，模糊/混合情绪最弱。模型仍有泛化标签偏置（`helplessness`、`anxiety`、`confusion`、`joy` 偏多），
  而 `hope`、`fear`、`disappointment` 偏少，情绪结果只能作为辅助信号，不能独立决定回复策略。
- 当前策略 `next_action` 会被拼入最终陪伴 LLM 的系统提示，但情绪标签本身不直接注入生成提示；`clarify` 有明确询问约束，
  其余策略仍属于软性指导。全部 170 条均为低风险样本，本轮不报告安全召回率；题集金标为场景设计标签，不代表真实用户客观情绪。
- FastAPI 与前端包版本同步更新为 `0.4.2`；README 已移除本机绝对路径、具体地址和端口，保留量化验证数据。
- 本版本不包含真实评测原文、运行日志、数据库、密钥或 NapCat 配置文件。
...
逐题 CSV、完整工具轨迹和 Judge 原始输出属于评测产物，不纳入公开仓库。
...
- 150 题的 Judge 使用当前模型自评；停止策略完整覆盖的是异常复测题和后续新增题，之前已正常完成的题
  保留原始基线结果，未全部重新运行。
- 当前评测结果用于个人 Agent 的工程基线，不代表公网多用户服务或独立人工金标结果。
...
Calls、在线 Embedding、BGE、FTS5/向量混合检索、在线 Reranker、长期记忆、JMComic 下载、
